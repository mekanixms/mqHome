...
def accelChangeCB(accel_xyz, gyro_xyz, roll, pitch, yaw, imu):
    global e
    e.send(str(int(remap(pitch - roll))) + ";"+str(int(remap(pitch + roll))))


a.addTrigger("accel_change", "BEFORE", accelChangeCB)
